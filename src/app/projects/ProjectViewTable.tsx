...
/**
 * Reusable table for flat view-based project lists (general, inah).
 * These views expose client_name / project_type_name as flat strings.
 */
...
export function ProjectViewTable({ data, emptyMessage = 'Sin proyectos' }: Props) {
            ...
            {!rows.length && (
              <tr>
                <td colSpan={6} className="text-center py-10 text-slate-400">
                  {filter ? 'Sin resultados para ese número de bitácora' : emptyMessage}
                </td>
              </tr>
            )}
            {rows.map((p) => (
              <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-5 py-3 text-slate-500">{p.project_type_name ?? '—'}</td>
                <td className="px-5 py-3">
